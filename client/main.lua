...
local mainMenu = RageUI.CreateMenu(Config.MenuTitle, _U('mainmenu_subtitle'), 0, 0, 'commonmenu', 'interaction_bgd', 255, 255, 255, 255)
...
function DrawPersonalMenu()
	ruiDrawContent(drawContentOptions, function()
		...
		RageUI.List(_U('mainmenu_gps_button'), PersonalMenu.GPSList, PersonalMenu.GPSIndex, nil, nil, true, function(Hovered, Active, Selected, Index)
	...
	for i = 1, #PersonalMenu.BillData do
		...
		RageUI.Button(billData.label, nil, { RightLabel = ('$%s'):format(GroupDigits(billData.amount)) }, true, function(Hovered, Active, Selected)
			...
			TriggerServerCallback('esx_billing:payBill', function()
				TriggerServerCallback('bpt_menu:Bill_getBills', function(bills) PersonalMenu.BillData = bills end)
			end, billData.id)
		end)
	end
end
...
getPersonalMenuCategory('vehicle').drawer = function()
	...
	RageUI.List(_U('vehicle_door_button'), PersonalMenu.DoorList, PersonalMenu.DoorIndex, nil, nil, true, function(Hovered, Active, Selected, Index)
...
getPersonalMenuCategory('boss').drawer = function()
	if societyMoney then
		RageUI.Button(_U('bossmanagement_chest_button'), nil, { RightLabel = ('$%s'):format(GroupDigits(societyMoney)) }, true, nil)
	end
...
if Config.DoubleJob then
	getPersonalMenuCategory('boss2').drawer = function()
		if societyMoney ~= nil then
			RageUI.Button(_U('bossmanagement2_chest_button'), nil, { RightLabel = ('$%s'):format(GroupDigits(societyMoney2)) }, true, nil)
		end
